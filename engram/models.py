...
class MemoryEntry(Base):
    ...
    compacted_into: Mapped[int | None] = mapped_column(
...
class Node(Base):
    """Universal entity store for agents, missions, artifacts, memory, and context."""
...
class Mission(Base):
    """Work units dispatched by a coordinator to agent instances."""

    __tablename__ = "missions"
    __table_args__ = (
        Index("idx_missions_status", "status"),
        Index("idx_missions_instance", "birka_instance_id"),
...
class AgentFleet(Base):
    """All active/dead agent instances managed by the coordinator."""

    __tablename__ = "agent_fleet"
    __table_args__ = (
    ...
    parent_instance_id: Mapped[str | None] = mapped_column(
        String(64), ForeignKey("agent_fleet.id"), nullable=True
    )
    ...
    # Relationships
    node: Mapped["Node"] = relationship("Node", foreign_keys=[node_id])
    parent: Mapped[Optional["AgentFleet"]] = relationship(
        "AgentFleet", remote_side="AgentFleet.id", back_populates="children"
    )
    children: Mapped[list["AgentFleet"]] = relationship(
        "AgentFleet", back_populates="parent"
    )
...
class SpawnTree(Base):
    ...
    parent_instance_id: Mapped[str] = mapped_column(
        String(64), ForeignKey("agent_fleet.id"), nullable=False
    )
    ...
    spawn_reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    ...
    # Relationships
    parent: Mapped["AgentFleet"] = relationship(
        "AgentFleet", foreign_keys=[parent_instance_id]
    )
    child: Mapped["AgentFleet"] = relationship(
        "AgentFleet", foreign_keys=[child_instance_id]
    )
...
class ResourceManifest(Base):
    """Agent capabilities declared by the instance at runtime."""
    ...
    # Relationships
    instance: Mapped["AgentFleet"] = relationship(
        "AgentFleet", foreign_keys=[instance_id]
    )


class ControlCommand(Base):
    """Signed commands from a coordinator to an agent instance."""
    ...
    # Relationships
    target: Mapped["AgentFleet"] = relationship(
        "AgentFleet", foreign_keys=[target_instance_id]
    )


class SafeguardEvent(Base):
    """Circuit breaker events (loops, depth exceeded, cost ceiling, timeout)."""
